...
def main_worker_stage1(args, log_s1_name, save_name=''):
    ...
    for epoch in range(args.epochs):
        ...
        if args.memorybank == 'CM':
            ...
            print('Unknown memory bank type!')
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            if args.use_global_clustering:
                ...
                rerank_dist_all = compute_jaccard_distance_xmodal(feature_all, k1=30, k2=6, search_option=5, all_file_name=all_fnames, rgbNum=len(all_rgb_fnames))  # change search_option
                pseudo_labels_all = cluster_rgb.fit_predict(rerank_dist_all)
                num_cluster_all = len(set(pseudo_labels_all)) - (1 if -1 in pseudo_labels_all else 0)
                num_outlier_all = len(np.where(pseudo_labels_all == -1)[0])
                print('==> Statistics for global clustering at epoch {}: {} clusters,  {}/{} outliers'.format(epoch, num_cluster_all, num_outlier_all, len(pseudo_labels_all)))
                del rerank_dist_all
        ...
        # ------------------------ global clustering related -----------------------------
        train_loader_global_ir, train_loader_global_rgb = None, None
        if args.use_global_clustering:
            proxy_features_all, proxy_labels_all, per_img_proxy_index = generate_modal_aware_proxy_features(pseudo_labels_all, feature_all, num_cluster_all, all_fnames)
            #proxy_features_all = generate_cluster_features(pseudo_labels_all, feature_all)
            #proxy_features_all = generate_modal_invariant_cluster_features(pseudo_labels_all, feature_all, num_cluster_all, all_fnames)
            #per_img_proxy_index = pseudo_labels_all

            proxy_features_all = F.normalize(proxy_features_all, dim=1)  # L2-normalize proxy features
            memory_all = ClusterMemory(model.module.num_features, num_cluster_all, temp=args.temp,
                                       momentum=args.momentum, mode=args.memorybank, has_multi_pos_loss=True).cuda()  # note here

            memory_all.proxy_pseudo_labels = proxy_labels_all.cuda()   
            if args.memorybank == 'CM':
                memory_all.features = proxy_features_all.cuda()
            elif args.memorybank == 'CMhybrid':
                memory_all.features = proxy_features_all.repeat(2, 1).cuda()

            sel_rgb_data = sorted(dataset_rgb.train)
            rgb_cnt = len(sel_rgb_data)
