...
from clustercontrast.trainers import ClusterContrastTrainer_Stage1, ClusterContrastTrainer_Stage2
...
from clustercontrast.utils.faiss_rerank_xmodal import compute_jaccard_distance_xmodal
from clustercontrast.utils.data.sampler import RandomMultipleGallerySampler, RandomMultipleGallerySamplerNoCam
from clustercontrast.utils.matching_and_clustering import subset_clustering
import os
...
def main_worker_stage1(args, log_s1_name, save_name=''):
    ...
    # Trainer
    trainer = ClusterContrastTrainer_Stage1(model)
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            if args.sample_ratio < 1:
                print('Clustering on random sampled RGB subsets, sample ratio= ', args.sample_ratio)
                subset_label_dict, index_to_subset_mapper, pseudo_labels_rgb, all_subset_inds = subset_clustering(
                    features_rgb, args.sample_ratio, cluster_algo_rgb, k1=args.k1, k2=args.k2, only_cluster_one_subset=True)
            else:
    ...
    for epoch in range(start_epoch, args.epochs):
        with torch.no_grad():
            ...
            if args.use_global_clustering:
                print('==> Create pseudo labels for unlabeled ALL(RGB+IR) data')
                all_ir_fnames = [f for f, _, _ in sorted(dataset_ir.train)]
                all_rgb_fnames = [f for f, _, _ in sorted(dataset_rgb.train)]
                if args.sample_ratio < 1:
                    subset_global = 1
                    features_all = torch.cat([features_rgb[all_subset_inds[subset_global]], features_ir], dim=0)  # RGB subset + IR full set feature
                    sel_rgb_fnames = [all_rgb_fnames[i] for i in all_subset_inds[subset_global]]
                else:
                    features_all = torch.cat([features_rgb, features_ir], dim=0)
                    sel_rgb_fnames = all_rgb_fnames
                all_fnames = sel_rgb_fnames + all_ir_fnames

                rerank_dist_all = compute_jaccard_distance_xmodal(features_all, k1=30, k2=6, search_option=5,
                                                                  all_file_name=all_fnames, rgbNum=len(sel_rgb_fnames))  # change search_option
                # rerank_dist_all = compute_modal_invariant_jaccard_distance(features_all, k1=40, k2=32,
                #                                            file=sorted(dataset_rgb.train) + sorted(dataset_ir.train), search_option=3)
                pseudo_labels_all = cluster_algo_all.fit_predict(rerank_dist_all)
                num_cluster_all = len(set(pseudo_labels_all)) - (1 if -1 in pseudo_labels_all else 0)
                print('==> Statistics for All(IR+RGB) epoch {}: {} clusters'.format(epoch, num_cluster_all))
                del rerank_dist_all
        ...
        @torch.no_grad()
        def generate_modal_aware_proxy_features(labels, features, num_clusters, all_fnames):
            centers_ir = collections.defaultdict(list)
            centers_ir_index = collections.defaultdict(list)
            centers_rgb = collections.defaultdict(list)
            centers_rgb_index = collections.defaultdict(list)
            for i, (label, fname) in enumerate(zip(labels, all_fnames)):
                if label == -1: continue
                if 'rgb_modify' in fname:
                    centers_rgb[labels[i]].append(features[i])
                    centers_rgb_index[labels[i]].append(i)
                elif 'ir_modify' in fname:
                    centers_ir[labels[i]].append(features[i])
                    centers_ir_index[labels[i]].append(i)
                else:
                    raise AssertionError
            all_centers, all_proxy_labels = [], []
            per_img_proxy_index = -1 * np.ones(labels.shape, dtype=np.int64)
            cnt = 0
            for lbl in range(0, num_clusters):
                if centers_rgb[lbl] != []:
                    all_centers.append(torch.stack(centers_rgb[lbl], dim=0).mean(0))
                    all_proxy_labels.append(lbl)
                    inds = np.array(centers_rgb_index[lbl])
                    per_img_proxy_index[inds] = cnt
                    cnt += 1
                if centers_ir[lbl] != []:
                    all_centers.append(torch.stack(centers_ir[lbl], dim=0).mean(0))
                    all_proxy_labels.append(lbl)
                    inds = np.array(centers_ir_index[lbl])
                    per_img_proxy_index[inds] = cnt
                    cnt += 1
            all_centers = torch.stack(all_centers, dim=0)
            all_proxy_labels = torch.tensor(all_proxy_labels)
            return all_centers, all_proxy_labels, per_img_proxy_index
        ...
        pseudo_labeled_dataset_ir = []
        ir_label = []
        for i, ((fname, pid, cid), label) in enumerate(zip(sorted(dataset_ir.train), pseudo_labels_ir)):
            if label != -1:
                pseudo_labeled_dataset_ir.append((fname, label.item(), cid, pid))
                ir_label.append(label.item())
        print('==> Statistics for IR epoch {}: {} clusters'.format(epoch, num_cluster_ir))

        pseudo_labeled_dataset_rgb = []
        rgb_label = []
        for i, ((fname, pid, cid), label) in enumerate(zip(sorted(dataset_rgb.train), pseudo_labels_rgb)):
            if label != -1:
                pseudo_labeled_dataset_rgb.append((fname, label.item(), cid, pid))
                rgb_label.append(label.item())
        print('==> Statistics for RGB epoch {}: {} clusters'.format(epoch, num_cluster_rgb))

        # ----------------- pseudo dataset for global clustering --------------------
        train_loader_global_ir, train_loader_global_rgb = None, None
        if args.use_global_clustering:
            cluster_features_all, proxy_labels_all, per_img_proxy_index = generate_modal_aware_proxy_features(
                pseudo_labels_all, features_all, num_cluster_all, all_fnames)
            # cluster_features_all = generate_modal_invariant_cluster_features(pseudo_labels_all, num_cluster_all,
            # features_all, sorted(dataset_rgb.train) + sorted(dataset_ir.train))
            memory_all = ClusterMemory(model.module.num_features, num_cluster_all, temp=args.temp,
                                       ...
                                       has_multi_pos_loss=True).cuda()
            memory_all.proxy_pseudo_labels = proxy_labels_all.cuda()
            memory_all.features = F.normalize(cluster_features_all.repeat(2, 1), dim=1).cuda()
            trainer.memory_all = memory_all

            if args.sample_ratio < 1:
                rgb_cnt = len(all_subset_inds[subset_global])
                sel_rgb_data = []
                ori_rgb_data = sorted(dataset_rgb.train)
                for i in all_subset_inds[subset_global]:
                    sel_rgb_data.append(ori_rgb_data[i])
            else:
                sel_rgb_data = sorted(dataset_rgb.train)
                rgb_cnt = len(sorted(dataset_rgb.train))

            pseudo_dataset_global_rgb = []
            for i, ((fname, _, cid), label, proxy_ind) in enumerate(
                    zip(sel_rgb_data, pseudo_labels_all[0:rgb_cnt], per_img_proxy_index[0:rgb_cnt])):
                if label != -1:
                    pseudo_dataset_global_rgb.append((fname, label.item(), cid, proxy_ind))

            pseudo_dataset_global_ir = []
            for i, ((fname, _, cid), label, proxy_ind) in enumerate(
                    zip(sorted(dataset_ir.train), pseudo_labels_all[rgb_cnt:], per_img_proxy_index[rgb_cnt:])):
                if label != -1:
                    pseudo_dataset_global_ir.append((fname, label.item(), cid, proxy_ind))

            train_loader_global_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                                         args.batch_size, args.workers, args.num_instances, iters,
                                                         trainset=pseudo_dataset_global_ir, no_cam=args.no_cam,
                                                         train_transformer=transform_thermal)

            train_loader_global_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                                             args.batch_size, args.workers, args.num_instances, iters,
                                                             trainset=pseudo_dataset_global_rgb, no_cam=args.no_cam,
                                                             train_transformer=train_transformer_rgb,
                                                             train_transformer1=train_transformer_rgb1)
            train_loader_global_ir.new_epoch()
            train_loader_global_rgb.new_epoch()
        # --------------------------------------------------------------------------
        ...
        trainer.train(epoch, train_loader_ir, train_loader_rgb, train_loader_global_ir, train_loader_global_rgb, optimizer,
                      print_freq=args.print_freq, train_iters=len(train_loader_ir), has_global_cluster_loss=args.use_global_clustering)
